        ...
        for i in range(1, len(self.frequence)):
            for key in list(self.frequence[i].keys()):
                ...
                for posterior in self.dfs_record(t, 1):
                    ...
                    now_lift = self.frequence[i][tuple(t)] / (self.frequence[i - 1][prior] * self.frequence[0][posterior])
                    if now_confidence > self.confidence:
                        self.res.append([prior, posterior, now_support, now_confidence,now_lift])

    def print_res(self):
        for item in self.res:
            print("%-40s -> %-30s Support: %4f,   Confidence: %4f,   Lift: %2f" %\
                (str(item[0]), str(item[1]), item[2], item[3], item[4]))
